...
export function Hero() {
  ...
  useGSAP(
    () => {
      ...
      const showAll = () => {
        gsap.set(
          root.querySelectorAll(
            "[data-hero-bg],[data-hero-badge],[data-hero-line],[data-hero-sub],[data-hero-actions],[data-hero-actions] a,[data-hero-wa],[data-hero-stat],[data-hero-scroll],[data-hero-image]",
          ),
          {
            autoAlpha: 1,
            y: 0,
      ...
      const ctx = gsap.context(() => {
        ...
        const imgPre = root.querySelector("[data-hero-image]");
        if (imgPre) gsap.set(imgPre, { autoAlpha: 0, scale: 1.06, rotate: -1.2 });
        ...
        const imgWrap = root.querySelector("[data-hero-image]");
        if (imgWrap) {
          tl.fromTo(
            imgWrap,
            { autoAlpha: 0, scale: 1.06, rotate: -1.2 },
            { autoAlpha: 1, scale: 1, rotate: 0, duration: 1.18, ease: "power3.out" },
            0.12,
          );
        }

        const parallaxEl = root.querySelector("[data-hero-parallax-inner]");
        if (parallaxEl) {
          gsap.fromTo(
            parallaxEl,
            { yPercent: -6 },
  ...
  return (
    <section
      ref={rootRef}
      className="relative overflow-hidden border-b border-[var(--border)] bg-[radial-gradient(120%_120%_at_100%_-20%,rgba(29,86,230,0.14)_0%,transparent_52%),linear-gradient(165deg,#f4f8fd_0%,#e8f1fb_28%,#ffffff_52%,#ffffff_100%)]"
    >
      <div
      ...
      <div
        data-hero-bg
        className="pointer-events-none absolute inset-0 opacity-[0.4] mix-blend-soft-light bg-[url('data:image/svg+xml,%3Csvg%20xmlns=%22http://www.w3.org/2000/svg%22%20width=%2260%22%20height=%2260%22%20viewBox=%220%200%2060%2060%22%3E%3Cg%20fill=%22none%22%20fill-rule=%22evenodd%22%3E%3Cg%20fill=%22%231d56e6%22%20fill-opacity=%220.06%22%3E%3Cpath%20d=%22M36%2034v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6%2034v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6%204V0H4v4H0v2h4v4h2V6h4V4H6z%22/%3E%3C/g%3E%3C/g%3E%3C/svg%3E')]"
      />
      <div
        data-hero-bg
        className="pointer-events-none absolute -right-32 top-0 h-[32rem] w-[32rem] rounded-full bg-[radial-gradient(circle,rgba(37,99,235,0.14)_0%,transparent_68%)] blur-3xl"
      />
      <div
        data-hero-bg
        className="pointer-events-none absolute -left-28 bottom-0 h-[22rem] w-[22rem] rounded-full bg-[radial-gradient(circle,rgba(14,165,233,0.16)_0%,transparent_70%)] blur-3xl"
      />
      ...
      <div className="relative mx-auto max-w-7xl px-4 pb-16 pt-14 sm:px-6 md:pb-20 md:pt-16 lg:px-8 lg:pb-24 lg:pt-20">
        <div className="grid min-w-0 items-center gap-10 lg:grid-cols-2 lg:gap-16">
          <div className="relative order-2 min-w-0 max-w-xl lg:order-1 lg:max-w-none">
            ...
            <ul
              className="mt-12 grid gap-4 rounded-2xl border border-white/90 bg-gradient-to-b from-white/85 to-white/65 p-6 backdrop-blur-md sm:grid-cols-3 sm:p-7"
